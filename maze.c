...
//Version beta del algoritmo para resolver el algoritmo (usando el stack) <-----LA MERA MEAR
...
//     stack* backtrack_stack = stack_create();
    
//     stack_push(rutaDeLaPila, entrada);
//     entrada->visited = 1;
    
//     node* current = entrada;

//     while (!stack_is_empty(rutaDeLaPila)) 
//     {
//         current = (node*)stack_pop(rutaDeLaPila);
//         stack_push(backtrack_stack, current);
...
//             return backtrack_stack;  //Retornar la ruta completa
//         }
...
//         //Si no hay vecinos válidos, hacer backtracking
//         if (vecinos_validos == 0 && !stack_is_empty(backtrack_stack)) 
//         {
//             //Retroceder hasta encontrar un nodo con más opciones
//             while (!stack_is_empty(backtrack_stack) && vecinos_validos == 0) 
//             {
//                 node* backtrack_node = (node*)stack_pop(backtrack_stack);
                
//                 //Verificar si este nodo tenía otros caminos
//                 node* backtrack_vecinos[] = {backtrack_node->up, backtrack_node->right, 
//                     backtrack_node->down, backtrack_node->left};

//                 vecinos_validos = 0;

//                 for (int i = 0; i < 4; i++) 
//                 {
//                     node* v = backtrack_vecinos[i];
//                     if (v != NULL && v->value == '0' && !v->visited) 
//                     {
//                         vecinos_validos++;
//                     }
//                 }
